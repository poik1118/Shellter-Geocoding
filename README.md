...
### 🔄 전체화면 모드
- 우측 상단의 **전체화면 버튼**을 클릭하여 전체화면으로 지도를 볼 수 있습니다
- 프레젠테이션이나 대화면에서 유용합니다

### 📱 모바일 지원
- 모바일 기기에서도 터치로 지도를 조작할 수 있습니다
...
### 🤔 **Q: 지도에서 마커가 보이지 않아요**
**A**: 다음을 확인해주세요:
- CSV 파일에 위도/경도 데이터가 있는지 확인
- 지도를 확대하여 마커가 보이는지 확인
- 브라우저의 JavaScript가 활성화되어 있는지 확인

### 🤔 **Q: 주소 좌표 변환이 실패해요**
**A**: 다음을 확인해주세요:
- `.env` 파일에 VWorld API 키가 올바르게 설정되어 있는지 확인
- VWorld API 키가 유효한지 확인
- 인터넷 연결이 안정적인지 확인
- API 호출 제한에 걸리지 않았는지 확인

### 🤔 **Q: VWorld API 키를 어떻게 발급받나요?**
**A**: 다음 단계를 따라주세요:
1. [VWorld](https://www.vworld.kr/)에 무료 가입
2. 로그인 후 **API 키 발급** 메뉴에서 키 생성
3. `.env` 파일에 키 정보 입력:
   ```
   VWORLD_API_KEY=your_vworld_api_key_here
   ```
...
### 🗺️ 지도 표시 관련
- **브라우저 호환성**: 최신 브라우저(Chrome, Firefox, Safari, Edge)에서 최적화되어 있습니다
- **인터넷 연결**: 지도 타일과 GeoJSON 데이터 다운로드를 위해 인터넷 연결이 필요합니다
- **JavaScript**: 지도의 모든 기능을 사용하려면 브라우저의 JavaScript가 활성화되어야 합니다

### 🔒 보안 관련
- **API 키 보호**: `.env` 파일은 `.gitignore`에 포함되어 Git에 업로드되지 않습니다
- **개인정보**: API 키는 절대 공개 저장소에 업로드하지 마세요
